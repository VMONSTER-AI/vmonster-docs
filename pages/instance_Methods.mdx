...
interface JoinOptions {
  ...
  streamId: string;
  token: string;
  unmuteUserAudioOnJoined?: boolean; // Whether the user’s audio track should be unmuted immediately upon the "joined" event. Defaults to False.
}
```
...
Connects the user’s audio track. STT starts immediately, and the `stt-start` event is triggered.

**Error Occurrence**

- An error will be triggered if the method is called while STT is already in progress.
...
This method mutes the user’s audio track. The `stt-stop` event is triggered.

The track remains connected, so STT can resume with minimal delay when the audio is unmuted again.

**Error Occurrence**

- An error will be triggered if the method is called while STT is not in progress.
...
- An error will be triggered if the method is called when the user’s audio is not published.
